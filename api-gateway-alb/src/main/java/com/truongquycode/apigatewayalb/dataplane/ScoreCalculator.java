...
public class ScoreCalculator {
     ...
     * instance này, tránh route traffic vào "hộp đen".
     * So sánh: finalScore bình thường ∈ [0.05, ~1.8].
     */
    private static final double SCORE_NULL_INSTANCE = 20.0;

    /**
     * Giá trị fallback cho P5 toàn hệ thống (ms) khi histogram chưa đủ data
     * (vừa reset hoặc hệ thống mới khởi động).
     * 30ms ≈ latency tối thiểu thực tế của một Java microservice có 50ms I/O.
     * Dùng làm cận dưới khi chuẩn hóa latency về [0,1].
     */
    private static final double SYSTEM_P5_FALLBACK = 30.0;   // ms

    /**
     * Giá trị fallback cho P95 toàn hệ thống (ms) khi histogram chưa đủ data.
     * 300ms ≈ giới hạn trên chấp nhận được của registration service khi bình thường.
     * Dùng làm cận trên khi chuẩn hóa: instance có ewmaLat = 300ms → nL = 1.0 (tệ nhất).
     */
     ...
     * - Per-instance: p50 làm fallback latency khi instance idle;
     *                 p95 dùng khi system snapshot không khả dụng;
     *                 qP99 làm ngưỡng chuẩn hóa queue.
     * - System-wide:  p5/p95 làm biên [min, max] cho Min-Max normalization latency;
     *                 p75 làm setpoint cho PID controller.
     */
    private final SlidingWindowManager windowManager;

    /**
     * Tính baseScore = α×nL + β×nQ + γ×nC.
     * Trọng số α/β/γ được DynamicWeightEngine cập nhật mỗi 5 giây
     * dựa trên Entropy Weight Method (EWM) + AHP.
     */
    private final DynamicWeightEngine weightEngine;

    /**
     * Tính penalty dựa trên PID controller per-instance.
     * Penalty tăng khi instance chậm hơn P75 hệ thống liên tục,
     * giảm dần khi instance hồi phục. Khoảng [0, lambda=0.8].
     */
    private final PIDController pidController;

    /**
     * Làm mượt latency thô bằng Adaptive EWMA (AEWMA).
     * τ (tau) tự động điều chỉnh: nhỏ khi latency spike (phản ứng nhanh),
     * lớn khi ổn định (lọc nhiễu). Tránh score dao động quá mạnh vì 1 request chậm.
     */
    private final EwmaSmoother ewmaSmoother;

    /**
     * Tập hợp các hàm chuẩn hóa (normalize) về khoảng [0, 1]:
     * - normalizeLatency(): Min-Max theo dải [sysP5, sysP95]
     * - normalizeQueue():   Log-scale theo qP99
     * - normalizeCpu():     Clamp đơn giản vào [0, 1]
     */
    private final NormalizationFunctions norm;

    /**
     * Đọc cấu hình EWMA (tauMin, tauMax, k) và PID (kp, ki, kd, ...) từ application.yml.
     * Cho phép thay đổi tham số mà không cần recompile.
     */
     ...
     * @param current    Raw metrics vừa poll được: latency (ms), queueLength, cpu [0,1]
     *                   Có thể null nếu poll thất bại.
    ...
    public ScoreBreakdown calculateScore(String instanceId, InstanceMetrics current) {
        ...
        if (current == null) {
            return new ScoreBreakdown(instanceId,
                    0,                  // ewmaLatency = 0 (không có data)
        ...
        double ewmaLat = ewmaSmoother.smooth(
                instanceId,
                lRaw,
                ewmaCfg.getTauMin(),  // τ tối thiểu (200ms) — phản ứng nhanh nhất
        ...
        //   ~0.05 → instance rất tốt (latency thấp, queue trống, CPU thấp, không bị PID phạt)
        //   ~1.8  → instance rất tệ (latency P95, queue đầy, CPU 100%, PID penalty tối đa)
        //
        // MetricsPoller sẽ áp thêm 1 lớp EMA lên finalScore này trước khi lưu vào MetricsCache,
        ...
        return new ScoreBreakdown(
                instanceId,
                ewmaLat,    // EWMA latency (ms) — giá trị thực sau smoothing
                nL,         // normLatency ∈ [0,1]
                nQ,         // normQueue   ∈ [0,1]
                nC,         // normCpu     ∈ [0,1]
