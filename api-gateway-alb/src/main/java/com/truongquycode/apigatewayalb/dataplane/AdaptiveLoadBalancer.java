...
public class AdaptiveLoadBalancer implements ReactorServiceInstanceLoadBalancer {
    ...
    // Crossover point: khi 1 node có thêm DELTA_CROSSOVER requests hơn node nhẹ nhất,
    ...
    // Trước: asymmetric 0.07/0.30 → alpha quá nhỏ khi giảm → scores hội tụ về bằng nhau
    ...
    // 15s warmup round-robin tạo ra nhiều requests tại baseline thấp → kéo median xuống
    ...
    private Response<ServiceInstance> selectBestInstance(List<ServiceInstance> instances) {
        ...
        record NodeInfo(ServiceInstance inst, double rawMcdm, double smoothedMcdm, int inflight,
                        boolean inWarmup) {}
        ...
        for (ServiceInstance inst : instances) {
            ...
            double         rawMcdm = (bd != null)
                ? Math.max(SCORE_FLOOR, bd.finalScore())
                : DEFAULT_SCORE;
        ...
        double   maxCapW    = 0;
        for (int i = 0; i < n; i++) {
            capWeights[i] = nodes.get(i).inWarmup()
                ? 1.0
                : 1.0 / Math.sqrt(Math.max(SCORE_FLOOR, nodes.get(i).smoothedMcdm()));
            if (capWeights[i] > maxCapW) maxCapW = capWeights[i];
        }
        ...
        for (int i = 0; i < n; i++) fairShare[i] = capWeights[i] / sumCap;
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (node.inflight() >= INFLIGHT_HARD_CAP) continue;
            ...
            double expectedInflight = totalInflight > 0
                ? totalInflight * fairShare[i]
                : 0.0;
            double excessRatio  = expectedInflight > 0
                ? Math.max(0.0, (double) node.inflight() / expectedInflight - 1.0)
                : 0.0;
